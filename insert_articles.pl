...
open(IN,"<:utf8","pk.xml") or die "can't open pk.xml\n";
...
while($line)
{
	...
	elsif($line =~ /<part pnum="(.*)" snum="(.*)" month="(.*)" year="(.*)" info="(.*)">/)
	{
		$part = $1;
		$snum = $2;
		$month = $3;
	...
	elsif($line =~ /<page>(.*)<\/page>/)
	{
		...
		{
			$count++;
			$id = "pk_" . $volume . "_" . $part . "_" . $page . "_" . $page_end . "_" . $count; 
		}
		else
		{
			$id = "pk_" . $volume . "_" . $part . "_" . $page . "_" . $page_end . "_0";
			$count = 0;
		}
		$prev_pages = $pages;
		if($page_end)
	...
	{
		insert_article($title,$authids,$author_name,$featid,$page,$page_end,$volume,$part,$snum,$year,$month,$info,$id);
		$authids = "";
		$featid = "";
		$author_name = "";
		$id = "";
...
sub insert_article()
{
	my($title,$authids,$author_name,$featid,$page,$page_end,$volume,$part,$snum,$year,$month,$info,$id) = @_;
	my($sth1);
	...
	$sth1=$dbh->prepare("insert into article values('$title','$authids','$author_name','$featid','$page','$page_end','$volume','$part','$snum','$year','$month','$info','$id')");
